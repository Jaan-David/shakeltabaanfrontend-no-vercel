...
const Reviews: React.FC<Props> = ({ productId, onReviewAdded }) => { // ✅ Added onReviewAdded to destructuring
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
      ...
      <section className="bg-white rounded-lg md:rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-4 md:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
          <h2 className="text-right text-base sm:text-lg md:text-xl font-bold text-blue-900">التعليقات والتقييمات</h2>
          <span className="text-xs sm:text-sm text-slate-600 bg-blue-50 px-2 py-1 rounded-full">({reviews.length})</span>
        </div>

        <div className="space-y-3 sm:space-y-4 max-h-[400px] sm:max-h-[500px] overflow-y-auto pr-1">
          {reviews.length === 0 ? (
            <div className="text-center py-6 sm:py-8 text-slate-500">
              <MessageCircle className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-2 sm:mb-3 text-slate-400" />
      ...
      <section className="bg-white rounded-lg md:rounded-2xl border border-gray-200 shadow-sm p-3 sm:p-4 md:p-6">
        <h2 className="text-base sm:text-lg font-bold mb-3 sm:mb-4 text-right text-blue-900">
        ...
        {authToken ? (
          ...
          <div className="text-center py-6 sm:py-8 text-slate-600 bg-blue-50 rounded-lg border-2 border-dashed border-blue-200">
            <LogIn className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-2 sm:mb-3 text-slate-400" />
