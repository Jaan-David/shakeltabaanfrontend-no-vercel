...
export const checkProductUnitConflict = (productId: string, newUnitType: UnitType): boolean => {
  const existingItem = clientCartItems.find(item => item.productId === productId);
  const existingUnitType = existingItem?.unit;
  const isComparableUnitType = existingUnitType === 'linear' || existingUnitType === 'cubic';
  
  if (existingItem && isComparableUnitType && existingUnitType !== newUnitType) {
...
export const cartService = {
  ...
  async getCart() {
    try {
        ...
        .map((i: any) => {
          ...
          if (storedItem) {
            try {
              ...
              unit = parsedItem.unitType || parsedItem.unit || 'linear';
            } catch (e) {
      ...
      // Handle 404 as "no cart yet" - this is normal for new users
  ...
  // Add item to cart
  async addToCart(item: AddToCartPayload) {
    try {
      if (!isAuthenticated()) {
        toast.error('الرجاء تسجيل الدخول أولاً', {
          position: "top-center",
      ...
      const payload = { productId: item.productId, itemQty: item.itemQty, unitType: item.unitType };
      ...
      const errorMessage = error?.response?.data?.message || 'فشل إضافة المنتج إلى عربة التسوق';
