...
import localProducts from '../../public/Test_data/products.json';
...
export const getProductUnitLabel = (product: Product): string => {
  ...
  if (product.IsCUBIC_METER) return 'متر مربع';
  return '';
};
...
export const getProductsWithState = async (): Promise<Product[]> => {
  ...
  try {
    ...
    const fallbackProducts = localProducts.map(product => processProductImagesStatic({
      ...product,
      ...
      tags: [],
      specifications: {},
      inStock: Boolean(product.status),
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString()
    }));

    globalProductsCache = fallbackProducts;
    cacheTimestamp = now;

    isLoadingProducts = false;
    pendingPromises.forEach(({ resolve }) => resolve(fallbackProducts));
    pendingPromises = [];
...
export const fetchProductsByCategory = async (categoryName: string): Promise<Product[]> => {
  ...
  try {
    ...
    console.log(`🔄 Using local fallback data for category: ${categoryName}`);
    const fallbackProducts = localProducts
      .filter(product => product.category === categoryName)
      .map(product => processProductImagesStatic({
        ...product,
        id: product.id.toString(),
        rating: 4.5,
        reviewsCount: 10,
