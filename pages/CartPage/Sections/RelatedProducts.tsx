      ...
      >
        {products.map((product) => (
          <div key={product.id} className="keen-slider__slide">
            <div className="bg-slate-800/50 backdrop-blur-sm rounded-[20px] shadow-sm border border-slate-700 p-4 mx-1 hover:shadow-md transition-shadow">
              <div className="aspect-square bg-slate-700/30 rounded-lg mb-3 overflow-hidden">
                <img 
                  src={product.image} 
                  alt={product.name}
              ...
              <h3 className="font-medium text-slate-100 text-sm mb-2 truncate" title={product.name}>
                {product.name}
              </h3>
              <div className="text-primary font-bold">{product.price.toLocaleString()} ج.م</div>
              <div className="text-xs text-slate-300 mt-1">{product.category}</div>
            </div>
          </div>
        ))}
      </div>
