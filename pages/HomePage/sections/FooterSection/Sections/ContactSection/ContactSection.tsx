    ...
    <div className="w-full sm:w-[80%] md:w-[60%] lg:w-[23%] min-h-0">
      ...
      <div className="text-left w-full mt-3 grid grid-cols-2 gap-x-4 gap-y-2 md:flex md:flex-col md:gap-y-3">
        {finalContacts.map((item, index) => (
            <div
             key={index}
             className="flex items-center justify-start gap-3 text-slate-200"
           >
             {/* icon */}
             {icons[item.type]}
             {/* content */}
             {Array.isArray(item.value) ? (
               <div className="text-sm text-left leading-snug text-slate-200">
                 {item.value.map((v, i) => (
                   <div key={i}>{v}</div>
                 ))}
               </div>
             ) : (
               <span className="text-sm text-left leading-snug text-slate-200">{item.value}</span>
             )}
           </div>
        ))}
      </div>
